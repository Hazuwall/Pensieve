...
namespace Pensieve
{
    public class Database : DatabaseBase
    {
        ...
        private Database()
            : base("Diary.db", new string[] { "badges", "docs", "notes", "notes_docs", "notes_songs", "notes_tags", "songs", "tags", "years" })
        {
        }
        ...
        public bool AddNote(Note Note)
        {
            ...
            using (var query = con.Prepare("INSERT OR ROLLBACK INTO notes VALUES (" + Note.ID + ",'" + Note.Date.ToString() + "'," + ((Note.IsImportant) ? 1 : 0) + ",'" + Note.Title.Shielding() + "','" + Note.Text.Shielding() + "','" + Note.Extra.Shielding() + "')"))
        ...
        public bool UpdateNote(long ID, Note Note)
        {
            ...
            using (var query = con.Prepare("UPDATE OR ROLLBACK notes SET id= " + Note.ID + ", date='" + Note.Date.ToString() + "', important=" + ((Note.IsImportant) ? 1 : 0) + ", title='" + Note.Title.Shielding() + "', text='" + Note.Text.Shielding() + "', extra='" + Note.Extra.Shielding() + "' WHERE id=" + ID))
                result = query.Step() == SQLiteResult.DONE;
        ...
        public Note GetNote(long ID)
        {
            using (var query = con.Prepare("SELECT date, important, title, text, extra FROM notes WHERE id=" + ID))
            {
                if (query.Step() != SQLiteResult.ROW)
                    return null;
                Note note = new Note();
                note.Date = SpecificDate.Parse((string)query[0]);
